...
/// Revision 4 timeline arithmetic, in the 3000-unit timescale the 30 fps probe
...
/// head handle.
///
/// Revision 3 then set the incoming clip's offset equal to the transition's, so
/// the two clips overlapped by half a transition. A `<spine>` is strictly
/// sequential and cannot represent overlapping siblings: Final Cut truncated
/// clip-a at clip-b's offset and re-appended the orphaned half second after
/// clip-b. Its returned file gives the rule — the clips **butt-join at the cut**
/// and only the transition is centred on it, drawing its overlap from the
/// handles either side.
public enum RoundTripSpikeTimeline {
    ...
    /// Timeline position of the edit between the two clips.
    public static var cutUnits: Int { clipDurationUnits }
    /// A centred transition starts half its duration before the cut. Confirmed
    /// by revision 3: Final Cut returned exactly this relation to the joint it
    /// settled on.
    public static var transitionOffsetUnits: Int { cutUnits - transitionUnits / 2 }
    /// The incoming clip begins *at the cut*, not where the transition begins.
    /// Revision 3 used the transition's offset here and Final Cut re-flowed the
    /// spine to remove the resulting overlap.
    public static var incomingOffsetUnits: Int { cutUnits }
...
public struct FCPXMLRoundTripSpikeBuilder: Sendable {
    ...
    private func makePlan(operationID: UUID, generatedAt: Date) -> FCPXMLRoundTripSpikePlan {
        FCPXMLRoundTripSpikePlan(
            schemaVersion: "1.1", probeRevision: 4, operationID: operationID, generatedAt: generatedAt, frameRate: "30 fps admission probe",
            probes: [
                .init(
                    projectName: "FCPCommandConsole Dissolve Admission Probe",
                    purpose: "Two-asset probe testing whether a fully specified Cross Dissolve survives the spine round trip intact. Revision 3 proved the effect is admitted; this revision changes exactly one number—the incoming clip butt-joins at the cut instead of overlapping the outgoing clip—so a failure has one possible cause.",
                    assumptions: [
                        "Asset admission passed in revisions 2 and 3; it is re-checked, not assumed.",
                        "Cross Dissolve native semantics passed in revision 3 and are expected to hold; re-checked, not assumed.",
                        "Spine geometry is unverified. The butt-join rule is derived from Final Cut's own returned file in revision 3, which is stronger evidence than revision 3's source, but it has not yet been observed to survive a round trip unaltered."
                    ]
                )
            ]
        )
    ...
    private func makeEvidence(operationID: UUID, generatedAt: Date) -> FCPXMLRoundTripSpikeEvidence {
        FCPXMLRoundTripSpikeEvidence(
            schemaVersion: "1.1", probeRevision: 4, operationID: operationID, generatedAt: generatedAt, predecessorFailure: predecessorFailure(),
            checks: [
                ...
                .init(name: "predecessor asset admission (revisions 2 and 3)", status: .pass, note: "Both passes imported without a crash; both assets resolved with real uid/sig, correct durations, and detected codecs."),
                .init(name: "predecessor bare transition (revision 2)", status: .fail, note: "Operation CA7D0733-A435-498E-BD82-149CFF863FC3: Final Cut returned the bare transition at offset=\"0s\" with enabled=\"0\" against a synthesized <effect uid=\"\"/>. A transition needs a real effect resource and a filter-video reference."),
                .init(name: "predecessor cross dissolve semantics (revision 3)", status: .pass, note: "Operation 6B8F8B1C-8171-4770-86C0-E5A859C3B32A on 2026-08-04: the transition returned with the exact UID sent, no enabled=\"0\", and all four params verbatim. Final Cut additionally synthesized an FFAudioTransition Audio Crossfade companion, which it does only for a genuinely instantiated transition."),
                .init(name: "predecessor spine geometry (revision 3)", status: .fail, note: "Revision 3 gave the incoming clip the transition's offset, overlapping the two spine clips by half a transition. A spine is strictly sequential: Final Cut truncated clip-a from 7s to 6.5s and re-appended the orphaned 0.5s after clip-b. This revision butt-joins the clips at the cut and centres only the transition on it."),
                ...
                .init(name: "cross dissolve native semantics", status: .unknown, note: "Re-checked, not inherited from revision 3: requires manual import and returned FCPXML inspection. The effect must come back with a non-empty uid and without enabled=\"0\"."),
                .init(name: "transition timing and handles", status: .unknown, note: "The open question of this revision. The returned spine must contain exactly two asset-clips—clip-a 0s to 7s and clip-b 7s to 14s—with the transition centred at 19500/3000s and no orphaned third element."),
    ...
    func makeFCPXML(mediaRecords: [FCPXMLRoundTripSpikeMediaRecord]) throws -> String {
        ...
        // Everything Final Cut has already accepted is kept byte-for-byte: the
        // assets, the asset-clip construction and the name-only format reference
        // from revision 2, and revision 3's effect resource, filter-video
        // reference, params, and centred transition offset. Revision 4 changes
        // one value — the incoming clip's offset — so that the clips butt-join
        // at the cut rather than overlapping. Overlapping spine siblings are not
        // representable and revision 3 was re-flowed because of it.
        return """
    ...
    private func readme(operationID: UUID) -> String {
        """
        # FCPCommandConsole Dissolve Admission Probe (Revision 4)

        Operation: \(operationID.uuidString)

        This is a syntax-validated probe—not an accepted Final Cut result. It did not automate Final Cut Pro, call a paid service, or upload media. Three predecessors are recorded in `evidence.json`: revision 1 crashed during import; revision 2 imported cleanly but its bare transition came back disabled at offset 0; revision 3's Cross Dissolve was admitted intact but its spine was re-flowed.

        Revision 4 keeps everything Final Cut has accepted so far—the assets, the asset-clip construction, the name-only format reference, and revision 3's `<effect>` resource carrying the Cross Dissolve UID `\(RoundTripSpikeCrossDissolve.uid)`, its `<filter-video>` reference, its params, and its centred transition offset—and changes exactly one value:

        - the incoming clip's `offset` is the cut (7s), not the transition's offset (6.5s), so the two spine clips butt-join instead of overlapping.

        A `<spine>` is strictly sequential. Revision 3 overlapped its two clips by half a transition, and Final Cut resolved that by truncating clip-a from 7s to 6.5s and re-appending the orphaned 0.5s after clip-b. Only the transition is centred on the joint; it draws its overlap from the one-second handles either side (clip-a uses source 0–7s of 8s, clip-b starts 1s into its source).
        ...
        3. Verify one project and two browser clips appear without an error or crash, then inspect whether a Cross Dissolve appears **between** the two clips at the 7-second mark, with no third clip after clip-b.
