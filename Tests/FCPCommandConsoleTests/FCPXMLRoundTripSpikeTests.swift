...
final class FCPXMLRoundTripSpikeTests: XCTestCase {
    ...
    func testBuildCreatesReducedDTDValidDissolveAdmissionPackage() throws {
        ...
        // Browser clips and assets are unchanged from revision 2, which Final
        ...
        for forbidden in ["living-still", "adjust-", "keyframe", "opacity", "color"] {
            XCTAssertFalse(xml.localizedCaseInsensitiveContains(forbidden), "Out-of-scope construct for a dissolve probe: \(forbidden)")
        }
    ...
    func testRevisionThreeTimelineIsFrameAlignedAndLeavesHandles() {
        ...
        for units in [RoundTripSpikeTimeline.clipDurationUnits, RoundTripSpikeTimeline.transitionUnits, RoundTripSpikeTimeline.transitionOffsetUnits, RoundTripSpikeTimeline.incomingOffsetUnits, RoundTripSpikeTimeline.incomingStartUnits] {
            XCTAssertEqual(units % frameUnits, 0, "\(units) is not frame-aligned at 30 fps")
        }
        // clip-a keeps a tail handle, clip-b keeps a head handle.
        XCTAssertEqual(sourceUnits - RoundTripSpikeTimeline.clipDurationUnits, RoundTripSpikeTimeline.handleUnits)
